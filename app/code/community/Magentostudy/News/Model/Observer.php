...
class Magentostudy_News_Model_Observer
{
     ...
     * If specified news post was added recently (term is defined in config)
    ...
    public function beforeNewsDisplayed(Varien_Event_Observer $observer)
    {
        ...
        $newsCreatedAt = Mage::app()->getLocale()
            ->date(strtotime($newsItem->getCreatedAt()));
        $daysDifference = $currentDate->sub($newsCreatedAt)
            ->getTimestamp() / (60 * 60 * 24);
